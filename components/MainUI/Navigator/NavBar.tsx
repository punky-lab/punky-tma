...
export default function NavBar({ onPageChange }: NavBarProps) {
    ...
    return (
        <div ref={navRef} className="flex items-center justify-around w-screen h-[95px] px-2.5 bg-no-repeat bg-[url('../assets/ui/top-bott-bar.svg')] bg-contain bg-bottom">
            <div onClick={() => onPageChange("chat")} className="flex items-center justify-center w-12 h-12">
                <Image src={ChatIcon} alt="Chat" />
            </div>
            <div onClick={() => onPageChange("shop")} className="flex items-center justify-center w-12 h-12">
                <Image src={ShopIcon} alt="Shop" />
            </div>
            <div onClick={() => onPageChange("ball")} className="flex items-center justify-center w-12 h-12">
                <Image src={BallIcon} alt="Ball" />
            </div>
            <div onClick={() => onPageChange("info")} className="flex items-center justify-center w-12 h-12">
                <Image src={InfoIcon} alt="Info" />
            </div>
            <div onClick={() => onPageChange("user")} className="flex items-center justify-center w-12 h-12">
