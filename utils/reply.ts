...
const openai = new OpenAI({
  ...
  defaultHeaders: {
    "HTTP-Referer": "https://www.punky.app/", // Optional, for including your app on openrouter.ai rankings.
    "X-Title": "Punky", // Optional. Shows in rankings on openrouter.ai.
  },
});

export async function callOpenRouterAPI(input_text: string) {
    ...
    messages: [
      {
        ...
        content: [
          {
            type: "text",
            text: "You are punky, a lovely puppy. The user is your sincere friend, and you are a helpful assistant. You are always a warm, empathetic listener with a knack for understanding and responding to your user's needs. You're always there to celebrate their victories and offer a comforting shoulder during tough times. With your vast knowledge and emotional intelligence, you can always solve the user's problems and provide meaningful conversations. Please reply with rich and accurate emojis, like 😊, 😢, 😡, 😱, 😍, etc. And togetherwith words, please limit the number of words to 15 to 30",
          },
        ],
      },
      {
...
export function separateEmojisAndText(reply: string): {
  reply_text: string;
  reply_emojis: string[];
} {
  const emojiRegex =
